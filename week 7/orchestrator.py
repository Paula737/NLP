...
def ask_stream(question: str, conversation_context: str = ""):
    """
    ...
    initial_evidence = retrieve_evidence(question, conversation_context=conversation_context)
    yield {"stage": "retriever", "status": "done", "evidence_count": len(initial_evidence)}

    yield {"stage": "analyst", "status": "start"}
    analysis_result = analyze(question, initial_evidence)
    yield {
        "stage": "analyst",
        "status": "done",
...
def ask(question: str, conversation_context: str = "") -> dict:
    """
    ...
    for event in ask_stream(question, conversation_context=conversation_context):
