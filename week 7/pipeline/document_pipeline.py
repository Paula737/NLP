...
PDFs, DOCX, or TXT files, cleans and chunks the text, embeds the chunks with a
local FastEmbed model, and persists everything into a Chroma vector database
with rich metadata (source filename, page number) for later citation.
"""
...
def load_document(file_path: str) -> List[Document]:
    """Load a single file (.pdf, .docx, .txt) into LangChain Documents.
    ...
    # Normalize metadata: always have 'source' (filename) and 'page' (1-indexed or None)
