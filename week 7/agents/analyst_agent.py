...
def request_more_evidence(refined_query: str, k: int = 6) -> List[EvidenceChunk]:
    ...
    """
    return retrieve_evidence(refined_query, final_size=k)
...
def build_analyst_agent():
    ...
    of the 5 tools to call, in what order, and how many times."""
    return create_agent(
        model=_llm(),
        tools=ANALYST_TOOLS,
...
def analyze(question: str, initial_evidence: List[EvidenceChunk]) -> AnalysisResult:
    """
    Runs the question + initial evidence through the real Analyst Agent.
    The agent itself decides, turn by turn, whether to call calculator,
    table_extractor, document_comparison, data_analysis, or
    request_more_evidence (the feedback loop) - or to just answer directly
    if the initial evidence is already sufficient.
    """
